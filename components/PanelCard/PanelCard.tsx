...
const PanelCard = ({ panelName, technicalDetails, address }: PanelCardProps) => {
  ...
  const getStatusColor = () => {
    ...
    if (efficiency >= 80) return "bg-gradient-to-br from-green-400 to-green-500 border-1 border-gray-100/50 shadow-sm shadow-green-500";
    if (efficiency >= 60) return "bg-gradient-to-br from-yellow-400 to-yellow-500 shadow-yellow-500";
      ...
      <div className="flex flex-row justify-between w-full h-full bg-[#F0EDEF] p-5 overflow-hidden relative">
          ...
          <div className="space-y-3">
            ...
            <div className="space-y-2">
              <div className="flex items-center text-sm text-gray-700">
                ...
                <span className="mr-1">{formatNumber(technicalDetails.capacity)} kW</span>
              </div>
              <div className="flex items-center text-sm text-gray-700">
                <IconWithBackground icon={Sun} color="#F59E0B" />
                <span className="font-medium ml-1">تولید امروز:</span> 
                <span className="mr-1">{formatNumber(technicalDetails.todayProduction)} kWh</span>
              </div>
              <div className="flex items-center text-sm text-gray-700">
                <IconWithBackground icon={Calendar} color="#3B82F6" />
                <span className="font-medium ml-1">بازدهی:</span> 
        ...
        <div className="flex flex-col gap-4 items-center z-10">
          ...
          <div className="flex flex-col items-center justify-center gap-2 p-3 rounded-2xl bg-[#F0F0F3] shadow-[inset_-4px_-4px_10px_rgba(255,255,255,0.8),inset_4px_4px_10px_rgba(0,0,0,0.1)] w-24">
            <div className={`h-4 w-4 rounded-full ${getStatusColor()} shadow-md`}></div>
